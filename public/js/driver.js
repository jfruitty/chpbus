...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('businsertForm').addEventListener('submit', async function (e) {
    ...
    try {
      const response = await fetch('/insertbusdriver', {
        method: 'POST',
        headers: {
  ...
  sendbutton.addEventListener('click', function () {
    ...
    fetch('/driversendtohr', {
      ...
      .then(response => {
        if (response.ok) {
          alert('ส่งถึง Hr สำเร็จครับ');
          window.location.reload();
        }
      })
      .catch(error => {
        console.error('Error removing bus:', error);
        alert('An error occurred while driversendtohr');
      })
      .finally(() => {
        setTimeout(() => {
...
async function fetchBusData() {
  try {
    const response = await fetch('/driverjson');
...
async function createtable() {
  ...
  if (!listenerdeletedAdded) {
    confirmYesBtn.addEventListener('click', async function () {
      ...
      try {
        ...
        const response = await fetch('/removebusdriver', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  if (!listenereditAdded) {
    document.getElementById('busEditForm').addEventListener('submit', async function (e) {
      ...
      try {
        const response = await fetch('/editbusdriver', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
