...
async function chpEnsureSchema() {
  await pool.query(`
    ...
    -- Append-only history of every HR-finalized dispatch (revives the old
    -- gateway behaviour that the chp refactor dropped). On each /sendmsgtodriver
    -- the current busfromhr/seatfromhr batch is COPIED here and never deleted,
    -- so /runhistory can show any past day. Keyed by service_date (the calendar
    -- date the bus actually ran) + create_at (when HR sent it). Tables already
    -- exist in prod; CREATE IF NOT EXISTS makes a fresh DB self-sufficient.
    CREATE TABLE IF NOT EXISTS chp.driverhistoryfromhr (
      id integer, driver_user_id varchar(255), per_id varchar(20),
      first_name varchar(100), last_name varchar(100), route varchar(255),
      day varchar(20), bound varchar(20), "time" varchar(20), bus_number integer,
      create_at timestamp DEFAULT CURRENT_TIMESTAMP
    );
    CREATE TABLE IF NOT EXISTS chp.seathistoryfromhr (
      id integer, userid varchar(255), perid varchar(20),
      first_name varchar(255), last_name varchar(255), route varchar(255),
      location varchar(255), day varchar(20), "time" varchar(20),
      busnumber integer, seat integer, bound varchar(20),
      create_at timestamp DEFAULT CURRENT_TIMESTAMP
    );
    ALTER TABLE chp.driverhistoryfromhr ADD COLUMN IF NOT EXISTS service_date date;
    ALTER TABLE chp.seathistoryfromhr   ADD COLUMN IF NOT EXISTS service_date date;
    CREATE INDEX IF NOT EXISTS driverhistoryfromhr_service_date_idx ON chp.driverhistoryfromhr (service_date);
    CREATE INDEX IF NOT EXISTS seathistoryfromhr_service_date_idx   ON chp.seathistoryfromhr (service_date);
...
// Append-only history of every HR-finalized dispatch. Copies the batch that was
// just written into busfromhr/seatfromhr into chp.driverhistoryfromhr /
// chp.seathistoryfromhr without ever deleting, so /runhistory can show any past
// day (keyed by service_date + create_at). Revives the old gateway
// inseat*HistoryFromHr() behaviour the chp refactor dropped. Atomic across both
// tables; the caller treats a failure as best-effort (the send has already gone
// out, so it must not surface as a 500).
async function chpAppendHistoryFromHr() {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    await client.query(`
      INSERT INTO chp.driverhistoryfromhr (
        driver_user_id, per_id, first_name, last_name,
        route, day, bound, "time", bus_number, service_date, create_at
      )
      SELECT
        driver_user_id, per_id, first_name, last_name,
        route, day, bound, "time", bus_number, service_date, now()
      FROM chp.busfromhr
    `);
    await client.query(`
      INSERT INTO chp.seathistoryfromhr (
        userid, perid, first_name, last_name, route, location,
        day, "time", busnumber, seat, bound, service_date, create_at
      )
      SELECT
        userid, perid, first_name, last_name, route, location,
        day, "time", busnumber, seat, bound, service_date, now()
      FROM chp.seatfromhr
    `);
    await client.query('COMMIT');
  } catch (err) {
    await client.query('ROLLBACK');
    console.error('chpAppendHistoryFromHr error:', err);
    throw err;
  } finally {
    client.release();
  }
...
// --- Full dispatch history (admin) -----------------------------------------
// /bushistory & /seathistory only show the latest from-hr batch (replace-all).
// This reads the append-only chp.driverhistoryfromhr / chp.seathistoryfromhr,
// so every day HR has ever finalized stays browsable. Grouped by service_date
// (the calendar day the bus actually ran); pick a day via ?date=YYYY-MM-DD.
app.get('/runhistory', requireRole('admin'), async (req, res) => {
  try {
    // Days that have any history, newest first. UNION both sides so a day shows
    // up even if (somehow) only one table got rows.
    const datesRes = await pool.query(`
      SELECT d FROM (
        SELECT DISTINCT to_char(service_date, 'YYYY-MM-DD') AS d
          FROM chp.driverhistoryfromhr WHERE service_date IS NOT NULL
        UNION
        SELECT DISTINCT to_char(service_date, 'YYYY-MM-DD') AS d
          FROM chp.seathistoryfromhr  WHERE service_date IS NOT NULL
      ) u ORDER BY d DESC
    `);
    const dates = datesRes.rows.map(r => r.d);
    const selected = (req.query.date && dates.includes(req.query.date))
      ? req.query.date
      : (dates[0] || null);
    ...
    let seatRows = [];
    if (selected) {
      busRows = (await pool.query(`
        SELECT per_id, first_name, last_name, route, day, bound, "time",
               bus_number, to_char(create_at, 'YYYY-MM-DD HH24:MI') AS sent_at
        FROM chp.driverhistoryfromhr
        WHERE to_char(service_date, 'YYYY-MM-DD') = $1
        ORDER BY route, bound, "time", bus_number
      `, [selected])).rows;
      seatRows = (await pool.query(`
        SELECT perid, first_name, last_name, route, location, day, bound, "time",
               busnumber, seat
        FROM chp.seathistoryfromhr
        WHERE to_char(service_date, 'YYYY-MM-DD') = $1
        ORDER BY route, bound, "time", busnumber, seat
      `, [selected])).rows;
    }
    res.render('runhistory', { title: 'ประวัติการจัดรถ', dates, selected, busRows, seatRows });
  } catch (err) {
    console.error('GET /runhistory error:', err);
    res.status(500).send('Server Error');
  }
});
...
app.post('/sendmsgtodriver', requireRole('admin'), async (req, res) => {
  try {
    ...
    // Preserve this batch in the append-only history for /runhistory. Best-effort:
    // the LINE messages are already sent, so a history write must not turn a
    // successful send into a 500. Failures are logged (→ error_log) and Telegram.
    try {
      await chpAppendHistoryFromHr();
    } catch (err) {
      console.error('chpAppendHistoryFromHr failed:', err);
      await telegramNotify(`⚠️ เก็บประวัติจัดรถ (runhistory) ไม่สำเร็จ: ${err.message}`).catch(() => {});
    }
