...
**Filter/sort/paging ของตาราง admin — [public/js/table-tools.js](public/js/table-tools.js) ตัวเดียวคุมทุกหน้า list** (filter → sort → paginate + MutationObserver รองรับ tbody ที่ re-render). filter-bar มี 3 โหมด:
- **column mode** — input มี `data-col="N"` (เลขคอลัมน์): filter รายคอลัมน์ AND กัน ใช้ที่ `driver`, `seatdriver`, `bustoday`, `seattoday` และ (เพิ่มแล้ว) `busfromhr`/`bushistory` (col 3,4,5,6) กับ `seatfromhr`/`seathistory` (col 3,5,6,7 — มีจุดขึ้นรถคั่น) เป็น 4 ช่อง สายรถ/วัน/ขา/เวลา เหมือนหน้า driver. logic filter เก่าใน `busfromhr.js`/`seatfromhr.js` ถูกถอดแล้ว (เหลือแค่ปุ่มดาวน์โหลด — สองไฟล์นี้ใช้ทั้งหน้า fromhr และ history)
- **grid mode** (booking dashboards) — input `data-filter="route|day|bound|time"` 4 ช่อง สายรถ/วัน/ขา/เวลา ใช้ที่ `thisweekdashboard`/`nextweekdashboard` ซึ่งวัน×ขา กางเป็น 14 คอลัมน์: สายรถ match กับ `data-route` บน `<tr>` (มาจาก `getDashboard`), วัน+ขา เลือกคอลัมน์ผ่าน `<th data-day="จันทร์" data-bound="ขาเข้า">`, เวลา match ค่าในเซลล์ — แถวติดเมื่อมี ≥1 เซลล์ที่เลือกซึ่ง**มีการจองจริง** (ไม่ใช่ ❌/ไม่ใช้) และตรงเวลา. โหมดนี้ยังฉีดช่องค้นหารวม (ค้นชื่อ/รหัส/แผนก) ควบคู่กันด้วย. `dashboard.js` เหลือแค่ dropdown อนุมัติแผนก + dblclick ซ่อนคอลัมน์ (filter/pagination เก่าถูกถอด, handler ไม่ส่ง `departments` แล้ว)
- **unified mode** — ไม่มีทั้ง data-col/data-filter: ซ่อน input เก่าแล้วฉีดช่องค้นหารวมช่องเดียว ใช้ที่ `member` ฯลฯ
